class InvFile < ActiveRecord::Base
  ...
  def bytestream_uri
    URI.parse("#{APP_CONFIG['uri_1']}#{self.inv_version.inv_object.node_number}/#{self.inv_version.inv_object.to_param}/#{self.inv_version.to_param}/#{self.to_param}?fixity=no")
  end
end
